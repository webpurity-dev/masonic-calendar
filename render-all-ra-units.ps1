# PowerShell script to generate a PDF for each Royal Arch Unit
# Usage: powershell -ExecutionPolicy Bypass -File .\render-all-ra-units.ps1
# Usage (first 2 units): powershell -ExecutionPolicy Bypass -File .\render-all-ra-units.ps1 -limit 2
...
$dataFile = Join-Path $scriptDir "document\data\royalarch_units.csv"
$consoleProject = Join-Path $scriptDir "src\MasonicCalendar.Console\MasonicCalendar.Console.csproj"
$outputDir = Join-Path $scriptDir "output"

# Verify files exist
if (-not (Test-Path $dataFile)) {
    Write-Host "Error: royalarch_units.csv not found"
    exit 1
}
...
# Read the CSV file
Write-Host "Reading Royal Arch units from CSV..."
...
if ($limit -gt 0) {
    ...
    Write-Host "Found $($units.Count) Royal Arch units (limited to $limit)"
}
else {
    Write-Host "Found $($units.Count) Royal Arch units"
}
Write-Host ""
