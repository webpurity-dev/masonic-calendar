# Render all unit sections (sections with id ending in "_units" and template "_data-driven/unit-page.html")
...
# Extract all sections with section_id ending in "_units" and template "_data-driven/unit-page.html"
$sections = @()
...
foreach ($block in $sectionBlocks) {
    ...
    if ($block -match 'section_id:\s*"([^"]+)"') {
        ...
        # Check if ends with "_units"
        if ($sectionId -like "*_units") {
            # Check if this section has template "_data-driven/unit-page.html"
            if ($block -match 'template:\s*"_data-driven/unit-page\.html"') {
                $sections += $sectionId
            }
        }
    }
...
Write-Host "[OK] Found $($sections.Count) unit section(s) to render:" -ForegroundColor Green
...
foreach ($section in $sections) {
    ...
    # Extract unit type from section ID (e.g., "craft_units" → "craft", "ra_units" → "ra")
    $unitTypeFromSection = $section -replace '_units$', ''
    
    $stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
    
    & dotnet run -- -template master_v1 -output $OutputFormat -section $section -outputfolder $unitTypeFromSection 2>&1 | Out-Null
