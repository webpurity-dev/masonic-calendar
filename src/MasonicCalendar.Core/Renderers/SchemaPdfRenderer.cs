...
if (window.Paged && typeof window.Paged.on === 'function') {
    ...
    /// v1.11: Load rank fixes configuration from data source mapping.
    /// </summary>
    private Task<RankFixes?> LoadRankFixesAsync(SectionConfig section, string masterTemplateKey)
    {
        if (_dataLoader == null || string.IsNullOrWhiteSpace(section.DataMapping))
        {
            if (_debugMode)
                Console.WriteLine($"    [LoadRankFixes] Skipping: DataLoader={_dataLoader != null}, DataMapping={section.DataMapping}");
        ...
        try
        {
            ...
            if (_debugMode && rankFixes != null && rankFixes.FixPpAbbreviations)
            {
